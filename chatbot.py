...
_QA = [
    # (keywords, handler_name) — first match wins, more specific first
...
def answer(question: str, ctx: dict) -> str:
    ...
    # hour-of-day phrasing should win over generic buy/sell routing
    ...
    if fn is None:
        return (
            "I'm not sure how to answer that yet. " + HELP
        )
    ...
    # Only append the timestamp to answers that contain prices/dates
    body = fn(ctx)
    return body + suffix
...
def _num(s: str) -> float | None:
    """Parse first $x.xx-style number out of an answer string."""
...
class _Handlers:
    ...
    def when_buy(ctx):
        ...
        if p.get("next_bottom") is not None:
            ...
            parts.append(
                f"- 🗓 Next swing bottom estimated around **{when}** "
                f"(median gap between bottoms on this coin: ~{p['bottom_gap_days']:.0f} days)."
            )
        lo, hi = p["entry_zone"]
        cur = p["current"]
        ref = ""
        ...
        if cur <= hi * 1.02:
            ...
            diff = (lo / cur - 1) * 100
            parts.append(
                f"💡 Price now is **{fmt(cur)}**, which is **above** the suggested zone "
                f"(~{abs(diff):.0f}% above). The plan is a *limit* order at the zone, not to chase here."
            )
    ...
    def entry_price(ctx):
        ...
        if cur <= hi * 1.02:
            ...
            status = (f"Current price is {fmt(cur)}, so this would be a limit order "
                      f"{(lo / cur - 1) * 100:.0f}%–{(hi / cur - 1) * 100:.0f}% below the current price.")
        return (
            f"**Entry price idea:** limit buy in the {_zone(p)} zone. {status}"
        )
    ...
    def when_sell(ctx):
        ...
        parts = ["**When to sell:**"]
        parts.append(
            f"- 🎯 Target **{fmt(p['sell_target'])}** "
            f"(~{p['leg_ret_pct']:+.0f}% above the entry zone — the median up-swing on this coin, "
    ...
    def sell_price(ctx):
        ...
        return (
            f"**Sell target:** around **{fmt(p['sell_target'])}**, with a sensible scale-out zone of "
            f"**{fmt(lo)} – {fmt(hi)}**."
        )
    ...
    def hold_time(ctx):
        ...
        if p.get("next_bottom") is not None and p.get("hold_until_est") is not None:
            extra = (f" For example, buy near the next expected bottom "
                     f"({fmt_d(p['next_bottom'])}) → target selling near "
                     f"**{fmt_d(p['hold_until_est'])}**.")
        return (
            f"**Typical hold time:** the median swing low→high run on {ctx['name']} "
            f"takes about **{p['hold_days']:.0f} days** (from {p['n_legs']} past swings)." + extra
        )
    ...
    def stop_loss(ctx):
        p = ctx["plan"]
        return (
            f"**Risk:** for an entry near {fmt(p['entry_price'])}, a stop around "
            f"**{fmt(p['stop_loss'])}** (just below the buy zone) implies "
            f"risk ≈ {p['risk_pct']:.0f}% vs. expected reward ≈ {p['reward_pct']:.0f}% "
            f"(risk-reward ~1:{p['risk_reward']:.1f})."
        )
    ...
    def best_day(ctx):
        return (
            f"For **{ctx['name']}**, historically: **{ctx['worst_day']}** is the weakest day "
            f"(avg {ctx.get('worst_day_ret', 0):+.2f}% — good for buying dips) and "
            f"**{ctx['best_day']}** is the strongest (avg {ctx.get('best_day_ret', 0):+.2f}% — good for selling into strength)."
        )
    ...
    def best_time(ctx):
        ...
        return (
            f"Historically for **{ctx['name']}** (your local time): intraday **lows tend to form near "
            f"{ctx['best_buy_hour']}** (good time to buy), and intraday **highs tend to form near "
            f"{ctx['best_sell_hour']}** (good time to sell)."
        )
    ...
    def plan(ctx):
        p = ctx["plan"]
        lo, hi = p["entry_zone"]
        entry_day = fmt_d(p["entry_day_ideal"]) if p.get("entry_day_ideal") is not None else (
            fmt_d(p["next_bottom"]) if p.get("next_bottom") is not None else "soon")
        ...
        return (
            ...
            f"\n3. **Sell** near **{fmt(p['sell_target'])}** "
            f"(~{p['leg_ret_pct']:+.0f}%) — historically a good day would be **{exit_day}**."
            f"\n4. **Stop loss** ~**{fmt(p['stop_loss'])}** below the zone "
            f"(risk {p['risk_pct']:.0f}% : reward {p['reward_pct']:.0f}%)."
        )
