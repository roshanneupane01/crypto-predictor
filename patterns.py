...
def _fetch_chunk(product_id: str, granularity: int, start: dt.datetime, end: dt.datetime) -> pd.DataFrame:
    frames = []
    ...
    while cur < end:
        ...
        try:
            raw = _get(f"/products/{product_id}/candles",
                params={"granularity": granularity, "start": cur.isoformat(), "end": chunk_end.isoformat()})
            if raw:
            ...
            import logging
            logging.warning(f"Failed to fetch candles for {product_id}: {e}")
        cur = chunk_end
    if not frames:
...
def _cached_history_uncached(product_id: str):
    return cache.get_cached(product_id, 86400), cache.get_cached(product_id, 3600)
...
def find_pivots(df: pd.DataFrame, order: int = 7, min_separation: int = 4, prominence_pct: float = 0.02):
    ...
    for i in range(order, n - order):
        ...
        if close[i] == w.max() and close[i] >= close[i - order] * (1 + prominence_pct) and close[i] >= close[i + order] * (1 + prominence_pct):
            hi.append(i)
        if close[i] == w.min() and close[i] <= close[i - order] * (1 - prominence_pct) and close[i] <= close[i + order] * (1 - prominence_pct):
            lo.append(i)
    def separate(idx, mode):
        ...
        for j in idx[1:]:
            if j - kept[-1] >= min_separation:
                kept.append(j)
            elif (mode == "max" and close[j] > close[kept[-1]]) or (mode == "min" and close[j] < close[kept[-1]]):
                kept[-1] = j
        return kept
...
def detect_runs(df: pd.DataFrame, threshold: float = 0.18):
    ...
    def _maybe_add(typ, i0, i1):
        ...
        if abs(ret) >= threshold * 100:
            runs.append({"type": typ, "start": pd.Timestamp(dates[i0]), "end": pd.Timestamp(dates[i1]),
                "start_price": float(p0), "end_price": float(p1),
                "days": int((pd.Timestamp(dates[i1]) - pd.Timestamp(dates[i0])).days), "return_pct": float(ret)})
    for i in range(1, n):
        v = prices[i]
    ...
    if direction == 1:
        ...
        if prices[end_i] > last_piv_p * 1.05:
            ...
            _maybe_add("bear", last_piv_i, end_i)
    cur_state = ((prices[-1] / run_lo - 1) if direction == 1 else (prices[-1] / run_hi - 1) if direction == -1 else 0.0)
...
def hourly_table(hourly: pd.DataFrame) -> pd.DataFrame:
    ...
    g = h.groupby("hour_local").agg(avg_ret=("ret", "mean"), avg_hi=("hi_from_open", "mean"), avg_lo=("lo_from_open", "mean"), samples=("ret", "count")).reset_index().sort_values("hour_local")
    return g
...
def today_summary(hourly: pd.DataFrame) -> dict:
    ...
    return {"low_price": float(t.loc[il, "low"]), "low_time": t.loc[il, "local_dt"],
...
def _best_future_weekday(avg_map: dict, from_dt: pd.Timestamp, sign: str, within_days: int = 45) -> pd.Timestamp:
...
def _recent_vol_factor(daily: pd.DataFrame) -> float:
    ...
    if long and not np.isnan(long) and long > 0:
        return float(np.clip(recent / long, 0.75, 1.5))
    return 1.0
...
def trade_plan(df: pd.DataFrame, ph: pd.DataFrame, pl: pd.DataFrame, price: float | None = None) -> dict:
    ...
    if len(pl) and len(ph):
        ...
        for i, p in enumerate(pdata):
            ...
            if nxt:
                legs.append({"hold_days": max(1, (nxt["date"] - p["date"]).days), "ret_pct": (nxt["price"] / p["price"] - 1) * 100})
    if len(legs) >= 4:
    ...
    supports = lows[(lows["price"] <= cur * 1.001) & (lows["price"] >= cur * 0.80)].sort_values("date")
    if len(supports) >= 1:
        ...
        support = min(support, cur)
        plan["support_ref"] = support
        lo = support * 0.985
        hi = support * 1.03
        hi = min(hi, cur * 1.005)
        if lo >= hi:
            lo, hi = min(lo, hi) * 0.995, max(lo, hi) * 1.005
        ...
        plan["entry_zone"] = (cur * 0.985, cur * 0.995)
        plan["entry_price"] = cur * 0.99
        plan["deep_discount"] = True
    ...
    sz_lo = max(e_hi * 1.01, plan["entry_price"] * (1 + plan["leg_ret_lo"] / 100))
    sz_hi = plan["entry_price"] * (1 + plan["leg_ret_hi"] / 100)
    plan["sell_zone"] = (sz_lo, sz_hi)
    plan["stop_loss"] = e_lo * 0.95
    plan["risk_pct"] = max(3.0, (plan["entry_price"] / plan["stop_loss"] - 1) * 100)
    plan["reward_pct"] = med_ret
    plan["risk_reward"] = min(med_ret / plan["risk_pct"], 10.0) if plan["risk_pct"] > 0 else float("inf")
    if len(ph):
...
def forecast_path(df: pd.DataFrame, plan: dict, days: int | None = None):
    ...
    base_rets = np.concatenate([np.cumsum(np.repeat(inc, min(hold_days, days))), np.repeat(target / entry - 1, max(0, days - hold_days))])[:days]
    noise = np.random.RandomState(42).normal(0, abs(inc) * 0.5, days)
    noise = np.cumsum(noise) * 0.1
    rets = base_rets + noise
    rets = np.clip(rets, -0.5, 2.0)
    path = entry * (1 + rets)
    path = np.maximum(path, entry * 0.5)
...
def signal_verdict(df: pd.DataFrame, plan: dict) -> dict:
    ...
    if cur <= e_hi * 1.03:
        if rsi < 32:
            return {"tone": "green", "emoji": "🟢", "verdict": "Inside/near a historical BUY zone with oversold momentum."}
        return {"tone": "green", "emoji": "🟢", "verdict": "Price is inside the suggested buy zone — historically a decent place to accumulate."}
    if plan.get("resistance_ref") and cur >= plan["resistance_ref"] * 0.97:
        return {"tone": "red", "emoji": "🔴", "verdict": "Near the recent swing high — historically where tops form."}
    if rsi > 68:
        return {"tone": "red", "emoji": "🔴", "verdict": "Momentum is overbought (RSI high) — chasing here has historically been poor."}
    if cur > e_hi * 1.25:
        return {"tone": "gray", "emoji": "⚪", "verdict": f"Price has run {plan['leg_ret_pct']:.0f}%-style moves before — mid-range, nothing obvious."}
    return {"tone": "gray", "emoji": "⚪", "verdict": "Mid-range relative to recent swings — fair to wait for a better zone."}
